...
export default function BucketModal({ onClose, data }) {
  ...
  const removeElement = (id) => {
    ...
    localStorage.removeItem('bucketData')
    localStorage.setItem('bucketData', updatedData)
    dispatch(remove(id))
  }
  ...
  return (
    <div
      className='fixed w-full h-full bg-[rgba(0,0,0,0.3)] flex items-center justify-center z-[1000]'
      onClick={onClose}
    >
      <div
        className='lg:rounded-xl bg-white shadow-lg px-8 py-4 w-[100vw] h-[100vh] lg:w-[60vw] lg:h-[75vh] max-h-[100vh] lg:max-h-[75vh]'
      ...
      >
              ...
              data.map((bucketItem, index) => (
                ...
                >
                  <div className='flex gap-4 items-center'>
                    {bucketItem.images && bucketItem.images.length && (
                      <img
                        src={bucketItem.images[0].link}
                  ...
                  <div className='flex gap-4 items-center text-[18px]'>
                    ...
                    <button
                      onClick={() => removeElement(bucketItem.id)}
                      className='border-2 border-redColor rounded p-2 bg-white transition duration-500 hover:bg-[rgb(255,99,71,0.3)]'
                    >
                      <img src={Trash} alt='remove element' />
                    </button>
                  </div>
                </div>
              ))
        ...
        <div className='w-full flex justify-end'>
          <div className='flex gap-4 items-center  mt-[25px]'>
            <span className='text-[21px] text-black'>
              {total}
              {t('value')}
            </span>
            <button
